...
| Custom Athena connector | `ATHENA_CONNECTOR` | Any source you can wrap in an Athena Query Federation SDK connector Lambda | Athena only (Lambda-backed catalog) |
...
!!! tip "Sources with no native support"
    ...
    `ATHENA_CONNECTOR` source. See
    [Custom Athena Connector Sources](athena-connector-sources.md) for what the
