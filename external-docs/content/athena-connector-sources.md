# Custom Athena Connector Sources
...
- [ ] Column comments emitted as **per-column Arrow field metadata**, with `@pk`/`@fk` tags where you have declared keys
...
!!! warning "Comments must be per-column Arrow field metadata"
    The SDK's schema builder writes **schema-level** metadata, which does not
    survive the path to `DESCRIBE`. Construct the Arrow fields directly and
    attach each comment to its own field. A connector whose comments are set at
    schema level looks identical to a connector that emits no comments at all:
    every description is AI-generated, and every `@pk`/`@fk` tag is lost with
...
policy alone. Two constraints apply, and both fail late if missed:

- **Do not name the function with this deployment's resource prefix.** The
  accelerator's roles carry an explicit `Deny` on `lambda:InvokeFunction` for
  in-account functions matching `<prefix>-*` when the call arrives through Athena.
  That `Deny` exists to stop an Athena user-defined function from reaching the
  accelerator's own Lambdas, and it cannot tell your connector apart from one of
  those by name alone. A connector named `<prefix>-my-connector` is refused.
- **Spilled responses are not supported in this topology.** The wildcard
  spill-read permission deliberately excludes the accelerator's own account, so a
  spill bucket you create there cannot be read and any response exceeding 6 MB
  fails with `AccessDenied`. Results under that limit are unaffected, which is
  what makes this easy to miss. Deploy the connector in a **separate account** if
  its responses can exceed 6 MB — that is the topology spill is designed for.
...
| 2 | Spill bucket | **Serve** role only | `s3:GetObject` | Whenever the connector has a spill bucket |
| 3 | Spill bucket's KMS key | **Serve** role only | `kms:Decrypt` | Whenever the connector has a spill bucket — SSE-KMS is required |

Only the serve role reads spilled data: discovery's `SHOW`/`DESCRIBE` traffic is
metadata-handler traffic, which never spills.
...
returns a spill location; Athena, running as the serve role, reads it to
assemble results. That read is a cross-account S3 read, so the bucket policy
must name the serve role:
...
```json
{
  ...
  "Statement": [{
    ...
    "Effect": "Allow",
    "Principal": { "AWS": "<serve-role-arn-from-ssm-above>" },
    "Action": "s3:GetObject",
...
!!! important "The spill bucket must use SSE-KMS with a tagged key"
    ...
    tag, a key policy granting the serve role `kms:Decrypt` with
...
and a `databaseSource.athenaConfiguration` body — see **CreateSource** in the
...
### Athena Connector Configuration Fields
...
sits on `databaseSource` alongside `athenaConfiguration`, not inside it — the
...
| Attach comments as **per-column Arrow field metadata** | The SDK's schema builder writes schema-level metadata only, which does not survive | Comments never reach the ontology; `@pk`/`@fk` tags are lost with them |
...
| Queries succeed until one fails `AccessDenied` | The spilled read is not authorized: `spill_prefix` is not `connectors/<connectorId>/spills`; the bucket is not SSE-KMS; its key lacks the `coa:connector-spill` tag; or the bucket/key policy does not name the serve role | Set `spill_prefix`; encrypt the bucket with a tagged customer-managed key; add the bucket and key policies |
| Column descriptions are all AI-generated although your connector emits comments | Comments were attached as schema-level Arrow metadata instead of per-column field metadata | Construct Arrow fields directly; re-create the source to re-discover |
...
- [ ] Comments attached as per-column Arrow field metadata; `@pk`/`@fk` tags encoded for declared keys
...
- [ ] Spill bucket policy grants `s3:GetObject` to the serve role
- [ ] Spill bucket encrypted with a customer-managed KMS key tagged `coa:connector-spill = true`
- [ ] That key's policy grants `kms:Decrypt` to the serve role with `kms:ViaService = s3.<region>.amazonaws.com`
