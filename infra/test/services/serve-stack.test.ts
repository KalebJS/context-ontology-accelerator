...
describe("ServeStack - custom Athena federation connector IAM", () => {
  ...
  beforeAll(() => {
    ...
    runtimeStatements =
      (runtimeRolePolicy as any).Properties.PolicyDocument.Statement;
  });
  ...
  it("does not grant invoke without the connector tag condition", () => {
    ...
    expect(condition.StringEquals).toHaveProperty("aws:ResourceTag/coa:connector");
  });
  ...
  // reaches every in-region Lambda in OUR account, including the
  // Lake-Formation-admin federation provisioner. Same-account invokes need no
  // resource policy, so nothing else would stop it.
  it("denies Athena-mediated invoke of this deployment's own functions", () => {
    expect(bySid("DenyAthenaInvokeOfOwnFunctions")).toEqual({
      Sid: "DenyAthenaInvokeOfOwnFunctions",
      Effect: "Deny",
      Action: "lambda:InvokeFunction",
      Resource: "arn:aws:lambda:us-east-1:123456789012:function:coa-*",
      // Conditioned, so it cannot touch the direct invokes this role makes
      // legitimately — the AOSS proxy Lambda shares this name prefix.
      Condition: {
        "ForAnyValue:StringEquals": {
          "aws:CalledVia": "athena.amazonaws.com",
        },
