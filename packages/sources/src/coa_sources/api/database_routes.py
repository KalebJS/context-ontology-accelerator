...
from coa_control_plane_server.models.athena_configuration import AthenaConfiguration
...
def _validate_athena_configuration(athena_config: Any) -> dict[str, Any] | None:
    ...
    """
    for field, arn in (("connectorFunctionArn", athena_config.connector_function_arn),):
        if not arn:
            continue
        ...
        if region != _AWS_REGION:
            return api_response(
                ...
                {
                    "error": (
                        f"athenaConfiguration.{field} must be in region '{_AWS_REGION}' "
...
def _resolve_query_engine(glue_config: Any, jdbc_config: Any) -> QueryEngine:
    ...
    A custom-connector (``ATHENA_CONNECTOR``) source passes neither config and
...
def _create_database_source(db_req: Any, namespace_id: str) -> dict[str, Any]:
    ...
    athena_config = db_req.athena_configuration
    supplied = [c for c in (glue_config, jdbc_config, athena_config) if c is not None]
    if not supplied:
        return api_response(
            400,
            {"error": "glueConfiguration, jdbcConfiguration or athenaConfiguration is required"},
        )
    ...
    if len(supplied) > 1:
        return api_response(
            400,
            {"error": "Provide exactly one of glueConfiguration, jdbcConfiguration or athenaConfiguration"},
        )

    if athena_config is not None:
        error = _validate_athena_configuration(athena_config)
        if error:
            return error
    ...
    # source's catalog.
    athena_catalog_name = derive_catalog_name(source_id) if athena_config else ""
    ...
    if athena_config:
        item["athenaDatabase"] = athena_config.database_name
    ...
    if athena_config:
        try:
            register_lambda_catalog(
                ...
                connector_function_arn=athena_config.connector_function_arn,
            )
...
def _apply_athena_configuration_update(
    item: dict[str, Any],
    new_config: dict[str, Any],
    update_fields: dict[str, Any],
) -> dict[str, Any] | None:
    """Guard and extend an ``athenaConfiguration`` update.
    ...
    * **``databaseName``.** Serve does not read ``athenaDatabase`` first — it prefers
    ...
    in it. The members stay in the request shape because ``AthenaConfiguration`` is
    ...
    for field in ("connectorFunctionArn",):
        ...
        if stored_arn != new_arn:
            return api_response(
                ...
                {
                    "error": (
                        f"athenaConfiguration.{field} cannot be changed after creation: the connector Lambda is "
                        f"bound into the registered Athena data catalog, which this update does not re-register. "
                        f"Delete the source and create it again with the new ARN."
                    )
                },
            )
    ...
    if new_database and stored_database and new_database != stored_database:
        return api_response(
            ...
            {
                "error": (
                    "athenaConfiguration.databaseName cannot be changed after creation: the discovered tables, "
                    "their approved metadata and the induced ontology all describe the current database, and "
                    "this update does not re-discover them. Delete the source and create it again against "
                    f"{new_database!r}."
                )
            },
        )
...
def _handle_update_metadata(event: dict[str, Any], namespace_id: str, source_id: str) -> dict[str, Any]:
    ...
    # ATHENA_CONNECTOR row (or vice versa) leaves a record whose blob and sub-type
    ...
    if supplied_config_keys:
        ...
        if error:
            return error
        if supplied_key == "athenaConfiguration":
            error = _apply_athena_configuration_update(item, body[supplied_key], update_fields)
            if error:
                return error
