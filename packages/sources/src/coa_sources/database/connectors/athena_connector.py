...
**Identifiers are unquoted.** Athena parses these statements with two different
parsers and they disagree on quoting: ``SHOW``/``DESCRIBE`` accept backticks and
reject double quotes, while ``SELECT`` accepts double quotes and rejects
backticks. Unquoted is the only form valid in both, so it is used uniformly. A
database or table name that genuinely requires quoting is therefore not
addressable — see :func:`_is_addressable_unquoted`, which reports such a name
rather than emitting a statement that would fail with a parser error.
"""
...
# Identifiers safe to send unquoted to both of Athena's parsers.
...
class AthenaConnector(MetadataConnector):
    ...
    def test_connection(self, config: dict) -> ConnectionTestResult:
            ...
            return ConnectionTestResult(
                ...
                checks=[
                    ConnectionCheck(
                        check="configuration",
                        status="failed",
                        message="athenaConfiguration.databaseName is required for this source type.",
                    )
                ],
            )
                ...
                return ConnectionTestResult(
                    ...
                    checks=[
                        ConnectionCheck(
                            ...
                            status="failed",
                            message=(
                                f"Athena's SHOW/DESCRIBE and SELECT parsers accept different quoting, so only "
                                f"unquoted identifiers work on both. Rename the {label} to letters, digits, and "
        ...
        return ConnectionTestResult(success=True, message="Athena connector reachable.", checks=checks)
    ...
    def discover_metadata(self, config: dict) -> DiscoveredMetadata:
        ...
        if not catalog or not database:
            raise ValueError("athena_data_catalog_name and database_name are required for an ATHENA_CONNECTOR source")
        for label, value in (("catalog", catalog), ("database", database)):
            if not _is_addressable_unquoted(value):
                raise ValueError(
                    f"Athena {label} name {value!r} needs quoting, which SHOW/DESCRIBE and SELECT spell "
                    f"differently; only unquoted identifiers work on both"
                )
        ...
        logger.info(
            "athena_connector_tables_listed",
            catalog=catalog,
            database=database,
        ...
        emit_metric("AthenaConnectorTablesDiscovered", len(tables), "Count")
        if failed:
            ...
            emit_metric("AthenaConnectorTablesFailed", len(failed), "Count")
            logger.warning(
                "athena_connector_tables_degraded",
                catalog=catalog,
                database=database,
    ...
    def _list_databases(self, catalog: str) -> set[str]:
        ...
        rows = self._runner.run(f"SHOW DATABASES IN {catalog}", header_row="absent")
        return {row[0] for row in rows if row and row[0]}

    def _list_tables(self, catalog: str, database: str) -> list[str]:
        """Table names from ``SHOW TABLES IN <catalog>.<database>``."""
        rows = self._runner.run(f"SHOW TABLES IN {catalog}.{database}", header_row="absent")
        return [row[0] for row in rows if row and row[0]]
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    logger.warning(
                        "athena_connector_describe_failed",
                        catalog=catalog,
                        database=database,
                        table=name,
                        exc_info=True,
                    ...
                    logger.warning(
                        "athena_connector_table_build_failed",
                        catalog=catalog,
                        database=database,
                        table=name,
                        exc_info=True,
                ...
                if table.columns:
                    ...
                    logger.warning(
                        "athena_connector_table_has_no_columns",
                        catalog=catalog,
                        database=database,
                        table=name,
                    )
    ...
    def _describe_one(self, catalog: str, database: str, table: str, config: dict) -> Table:
        ...
        if not _is_addressable_unquoted(table):
            # Recorded as a failed table rather than skipped silently: the steward
            # needs to know the connector exposes something we cannot address.
            raise ValueError(f"table name {table!r} needs quoting and cannot be addressed unquoted")
        rows = self._runner.run(f"DESCRIBE {catalog}.{database}.{table}", header_row="absent")
