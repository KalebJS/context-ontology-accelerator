...
class TestAthenaConnectorBranch:
    ...
    _ITEM = {"sourceSubType": "ATHENA_CONNECTOR", "athenaDataCatalogName": "coadevds_abc123"}
    ...
    def test_marks_the_source_queryable(self):
        ...
        with ctx:
            out = handler(_EVENT)
        assert out == {"provisioned": False, "reason": "athena-connector", "queryable": True}
        assert dao.update.call_args.kwargs["update_fields"] == {"queryable": True}
