...
# ===================================================================
# _create_database_source — ATHENA_CONNECTOR (custom connector)
# ===================================================================
...
def _make_athena_db_req(
    ...
    req.glue_configuration = None
    req.jdbc_configuration = None
    req.athena_configuration = MagicMock()
    req.athena_configuration.connector_function_arn = metadata_arn
    req.athena_configuration.database_name = database_name
    req.athena_configuration.to_dict.return_value = {
...
@pytest.mark.unit
class TestCreateAthenaConnectorSource:
    ...
    def test_derives_the_athena_connector_sub_type(self):
        status, _, mock_dao, _, _ = self._create(_make_athena_db_req())
        assert status == 202
        assert mock_dao.put.call_args[0][0]["sourceSubType"] == "ATHENA_CONNECTOR"
    ...
    def test_error_names_athena_configuration_when_no_config_is_given(self):
        req = _make_athena_db_req()
        req.athena_configuration = None
        status, body, _, _, _ = self._create(req)
        assert status == 400
        assert "athenaConfiguration" in body["error"]
...
@pytest.mark.unit
class TestUpdateAthenaConfiguration:
    ...
    def test_ignores_an_unmodelled_record_function_arn(self):
        ...
        The member was removed from AthenaConfiguration, so a client still sending it is
    ...
    def test_rejects_a_changed_database_name(self):
        status, body, mock_dao = self._update(
            {"athenaConfiguration": {"connectorFunctionArn": self._ARN, "databaseName": "gadgets"}}
        )
        assert status == 400
    ...
    def test_allows_an_unchanged_database_name(self):
        status, _, mock_dao = self._update(
            {"athenaConfiguration": {"connectorFunctionArn": self._ARN, "databaseName": "widgets"}}
        )
        assert status == 200
        mock_dao.update.assert_called_once()
    ...
    def test_mirrors_the_database_name_when_the_attribute_is_absent(self):
        ...
        status, _, mock_dao = self._update(
            {"athenaConfiguration": {"connectorFunctionArn": self._ARN, "databaseName": "widgets"}}, item=item
        )
        assert status == 200
    ...
    # A Glue config on an ATHENA_CONNECTOR row would leave the blob and the
