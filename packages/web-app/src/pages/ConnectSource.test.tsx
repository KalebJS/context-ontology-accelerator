...
/** Select the Athena connector tile and fill its required step-2 fields. */
async function fillAthenaConnectorStep() {
  await user.click(screen.getByLabelText("Athena connector"));
  await clickNext();
  await user.type(
    screen.getByPlaceholderText("My Athena Connector"),
    "my-connector",
  );
...
describe("ConnectSource — Athena Query Federation connector", () => {
  ...
  it("submits athenaConfiguration and nothing else", async () => {
    render(<ConnectSource />, { wrapper });
    ...
    expect(databaseSource.athenaConfiguration).toEqual({
  ...
  it("submits the table filters when provided", async () => {
    ...
    const cfg =
      mockCreate.mock.calls[0][0].body.databaseSource.athenaConfiguration;
