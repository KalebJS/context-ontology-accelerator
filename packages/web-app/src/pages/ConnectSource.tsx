...
interface Model {
  ...
  athenaName: string;
  athenaConnectorFunctionArn: string;
  athenaDatabaseName: string;
  athenaTableFilter: string;
  athenaTableExcludeFilter: string;
...
const initialModel: Model = {
  ...
  jdbcExternalId: "",
  athenaName: "",
  athenaConnectorFunctionArn: "",
  athenaDatabaseName: "",
  athenaTableFilter: "",
  athenaTableExcludeFilter: "",
...
function sourceNameFor(model: Model, docTab: "upload" | "s3"): string {
  switch (model.sourceKind) {
    ...
    case "ATHENA_CONNECTOR":
      return model.athenaName;
    case "DOCUMENTS":
...
function validateStep2(
  ...
  } else if (model.sourceKind === "ATHENA_CONNECTOR") {
    if (!model.athenaName.trim()) errs.athenaName = "Name is required.";
    if (!model.athenaConnectorFunctionArn.trim())
      errs.athenaConnectorFunctionArn = "Connector function ARN is required.";
    else {
      const arnErr = validateLambdaFunctionArn(
        model.athenaConnectorFunctionArn,
      );
      if (arnErr) errs.athenaConnectorFunctionArn = arnErr;
    }
    if (!model.athenaDatabaseName.trim())
      errs.athenaDatabaseName = "Database name is required.";
  } else {
    // DOCUMENTS
    if (docTab === "upload") {
...
export const ConnectSource: React.FC = () => {
  ...
  const isDatabase =
    ...
    model.sourceKind === "ATHENA_CONNECTOR";
  ...
  const handleSubmit = async () => {
    ...
    if (model.sourceKind === "ATHENA_CONNECTOR") {
      createSource({
        namespaceId: namespaceId!,
        body: {
          sourceType: SourceType.DATABASE,
          databaseSource: {
            name: model.athenaName.trim(),
            metadataEnrichmentEnabled: model.metadataEnrichmentEnabled,
            // athenaConfiguration only: the backend rejects a payload carrying
            // more than one of glue/jdbc/athena configuration, since the
            // configuration present is what selects the sub-type.
            athenaConfiguration: {
              connectorFunctionArn: model.athenaConnectorFunctionArn.trim(),
              databaseName: model.athenaDatabaseName.trim(),
              tableFilter: model.athenaTableFilter.trim() || undefined,
              tableExcludeFilter:
                model.athenaTableExcludeFilter.trim() || undefined,
            },
          },
        },
      });
        ...
        <Container header={<Header variant="h2">Source type</Header>}>
          <Tiles
            ...
            items={[
              ...
              {
                value: "ATHENA_CONNECTOR",
                label: SOURCE_KIND_LABELS.ATHENA_CONNECTOR,
                description:
          ...
          <SpaceBetween size="l">
            <Container header={<Header variant="h2">Source details</Header>}>
              <FormField
                ...
                description="A unique name for this source within the namespace."
                errorText={step2Errs.athenaName}
              >
                <Input
                  value={model.athenaName}
                  onChange={({ detail }) =>
                    setField("athenaName", detail.value)
                  }
                  placeholder="My Athena Connector"
                />
              </FormField>
            </Container>
            <Container header={<Header variant="h2">Connector</Header>}>
              <SpaceBetween size="l">
                <FormField
                  ...
                  errorText={step2Errs.athenaConnectorFunctionArn}
                >
                  <Input
                    value={model.athenaConnectorFunctionArn}
                    onChange={({ detail }) =>
                      setField("athenaConnectorFunctionArn", detail.value)
                    }
                ...
                <FormField
                  ...
                  errorText={step2Errs.athenaDatabaseName}
                >
                  <Input
                    value={model.athenaDatabaseName}
                    onChange={({ detail }) =>
                      setField("athenaDatabaseName", detail.value)
                    }
            ...
            >
              <SpaceBetween size="l">
                ...
                >
                  <Input
                    value={model.athenaTableFilter}
                    onChange={({ detail }) =>
                      setField("athenaTableFilter", detail.value)
                    }
                    placeholder="orders|customers"
                  />
                </FormField>
                ...
                >
                  <Input
                    value={model.athenaTableExcludeFilter}
                    onChange={({ detail }) =>
                      setField("athenaTableExcludeFilter", detail.value)
                    }
                    placeholder="tmp_*|staging_*"
                  />
                </FormField>
            ...
            <Container header={<Header variant="h2">Connection</Header>}>
              <KeyValuePairs
                ...
                items={[
                  ...
                  { label: "Database", value: model.athenaDatabaseName },
                  {
                    label: "Table filter",
                    value: model.athenaTableFilter || "(none)",
                  },
                ]}
              />
            </Container>
