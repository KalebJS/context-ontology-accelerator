    ...
    data: {
      body: {
        ...
        databaseDetails: {
          ...
          athenaConfiguration: ATHENA_CONFIG,
        },
        ...overrides,
      },
    },
...
describe("SourceDetail — ATHENA_CONNECTOR sub-type", () => {
  beforeEach(() => {
  ...
  it("labels the sub-type as an Athena connector rather than a JDBC database", () => {
    setSource();
    render(<SourceDetail />, { wrapper });

    expect(screen.getByText("Athena connector")).toBeInTheDocument();
  ...
  it("renders the connector settings panel instead of an empty container", async () => {
    ...
    expect(
      screen.getByText(ATHENA_CONFIG.connectorFunctionArn),
    ).toBeInTheDocument();
  ...
  it("shows exactly one connector ARN, with no metadata/record split", async () => {
    ...
    // metadata/record pair and AthenaConfiguration deliberately does not model it,
