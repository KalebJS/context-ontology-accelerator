...
Lambda in their own account, registers it as an Athena data catalog, and onboards it by
catalog name. This folder holds two toolkits, a working reference connector, and its CDK app.
...
2. Implement three methods in your metadata handler, subclassing `CoaMetadataHandler`:
   `listDatabases()`, `listTables(database)`, `describeTable(database, table)` — the last returns a
   `CoaTable` of `CoaColumn`s. Then a `RecordHandler` that writes rows, and a `CompositeHandler`
   wiring the two together. `example/src/main/java/dev/coa/example/` is all three, working.
...
5. Register the Athena catalog in the account that will query it — the stack prints the exact
   command as its `RegisterCatalogCommand` output — then onboard it in COA by catalog name.

6. Confirm the keys arrived, which is the one thing that fails silently:

```sql
DESCRIBE my-source.my_db.my_table;   -- the comment column must show your prose plus @pk / @fk(...)
```
...
public class YourMetadataHandler extends CoaMetadataHandler {
    ...
    protected List<String> listDatabases() { return List.of("your_db"); }
...
Then [register the catalog](#register-the-data-catalog-yourself) in the account that will
query it.
...
can be an explicit grant rather than something a wildcard already permitted. See constraint 11
for the one case it excludes.
...
### Register the data catalog yourself

The stack deliberately does **not** create an `AWS::Athena::DataCatalog`. The catalog belongs
in the account that runs the queries, which in the cross-account topology these connectors
are built for is not the account the Lambda lives in. The stack outputs the command:
...
[register the catalog](#register-the-data-catalog-yourself) in the account that will query it,
and confirm the tags actually arrived:
...
10. **`coa-contract.ts` states one side of a contract whose other side is elsewhere.** The tag
    ...
    over 6 MB.
11. **COA's spill-read grant excludes its own account.** The serve role's
    `AthenaFederationSpillRead` statement carries
    `StringNotEquals aws:ResourceAccount = <COA's account>`, deliberately, so that a
    wildcard `s3:GetObject` on a connector spill path cannot be composed into a way to read
    COA's own buckets. Cross-account deployment — the topology these connectors
    are built for — is unaffected. Deploying a connector into the *same* account as the
    COA falls outside that grant, and there the bucket policy this construct writes is
    what carries access, since a same-account request needs an allow from either the identity or
    the resource policy rather than both. That path has not been exercised end to end.
